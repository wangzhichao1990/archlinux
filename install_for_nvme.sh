...
then
    ...
    mkfs.fat -F32 /dev/${DISK}p1 1> /dev/null 2> ./errorfile || funerror "mkfserror" 4
    mkfs.ext4 /dev/${DISK}p2 1> /dev/null 2> ./errorfile || funerror "mkfserror" 4
    mount /dev/${DISK}p2 /mnt && mkdir /mnt/boot && mount /dev/${DISK}p1 /mnt/boot
else
    ...
    mkfs.ext4 /dev/${DISK}p1 1> /dev/null 2> ./errorfile || funerror "mkfserror" 4
